...
make > /dev/null
> results# check if there is an arg $1 for the quantity if not set quantity as 5
if [ -z "$1" ]
  then
    quantity=5
  else
    quantity=$1
fi# check if there is a second arg $2 for the number of tests if not set quantity as 100
if [ -z "$2" ]
  then
    tests=100
  else
    tests=$2
fi# Function to generate a random number between -50 and 50
...
for ((i=1; i<=tests; i++)); do
    ...
    total_moves=$((total_moves + moves))   # Check if the moves meet the criteria based on quantity
    case $quantity in
        5)
            max_moves=12
            ;;
        ...
        *)
            ...
            ;;
    esac    if [ $moves -le $max_moves ]; then
...
done    average_moves=$(echo "scale=2; $total_moves / $tests" | bc)
